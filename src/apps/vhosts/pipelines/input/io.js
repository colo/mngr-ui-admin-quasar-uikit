...
export default new Class({
  ...
  options: {
    ...
    io: {
			...
			// rooms: ['vhosts'], //atomatically join connected sockets to this rooms
			routes: {
				// 'app.doc': [{
				// 	// path: ':param',
  ...
  //
  // },
  vhosts: function(socket, next, doc){
    // let {type} = doc
    debug('vhosts %o', doc)

    if(doc.status){
      debug('ERROR vhosts %o', doc)
    }
    ...
    //   debug('vhosts %o', doc)
    //   // Array.each(doc.vhosts.tags, function(tag){
    //   //   debug('TAG %s', tag)
    ...
    //   // }.bind(this))
    //   //
    //   // Array.each(doc.vhosts.hosts, function(host){
    //   //   debug('HOST %s', host)
    //   //   // this.io.emit('/tags/'+tag)
    //   //   this.io.emit('/', {
    //   //     params: { prop: 'hosts' },
    ...
    // }
    // else{
    //   debug('OTHERS vhosts %o', doc)
    // }

    this.fireEvent('onDoc', [doc, { input_type: this, app: null }])

    // store.commit('vhosts/clear')
    // store.commit('vhosts/set', doc[type])
  },
  ...
  //
	// 	// arguments[1]()
	// 	// this.io.to('vhosts').emit('response', 'a new user has joined the room saying '+arguments[2]);
	// 	// next(socket)
	// },
  ...
  initialize: function(options){
    ...
    this.addEvent('onExit', function(){
      ...
      // if(this.io.disconnected == false)
		...
		this.profile('vhosts_init');//end profiling

		this.log('vhosts', 'info', 'vhosts started');
  },
