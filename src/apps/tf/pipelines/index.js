...
const debug = Debug('apps:tf:pipelines')
...
export default {
  ...
  output: [
    function (payload) {
      if (!payload.err && /^input\.tf\[.*\]$/.test(payload.id)) {
        payload.id = payload.id.replace('input.tf[', '').slice(0, -1)
        debug('OUTPUT', payload)
        EventBus.$emit('input.tf.' + payload.metadata.input, payload)
      }
